...
```{r}
PS2 = filter_taxa(phyloseq_obj, function(x) sum(x > 3) > (0.2*length(x)), TRUE)

sample_data(PS2)$human = factor( get_variable(PS2, "env_feature_s") %in% c("human-associated habitat") )
...
human = get_variable(PS2, "env_feature_s") %in% c("human-associated habitat")
sample_data(PS2)$human <- factor(human)
