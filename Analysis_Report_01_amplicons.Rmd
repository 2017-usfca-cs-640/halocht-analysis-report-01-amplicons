...
# Introduction
   ...
   Complete quantification and representation of the bacterial diversity inherently present in the entire human microbiota represents a daunting task transcending multiple scales of magnitude: Bacterial diversity is directly influenced by factors such as the diet, lifestyle, and environmental conditions of the human host and indirectly influenced by factors such as the relationship dynamic between the communities present, the morphology and structure of the area where they reside, and the disease profile of the host (Grice et. al, 2008; Ying et. al, 2015). Furthermore, bacterial diversity also relies on the structure and composition of the commnuity itself, which differs greatly between individual sites on the same host (Ying et. al., 2015). Not only are most of the bacterial species unable to be cultured, but multiple factors  intricate ecological system that is both dependent on individual human differences (genetics, metabolics, and lifestyle-associated factors) as well as the inherent relationships present among the thousands of bacteria actively competing for residency within a human host. Although researchers have verified that the human body possesses micro-environments where differences between two places in the body in terms of bacterial abundance can be vast, the human skin has been a recent focus.
  ...
  3. Regions of skin can vary substantially based on humidity, temperature, morphology, and exposure to elements which directly influence how bacteria can grow on different surfaces.
  
  Of particular interest in the subset of human skin, are microbial communities residing on our hands. Our hands represent an interesting conjunction of these factors because they can be hairy or not hairy, sweat more than normal, are constantly exposed to the environment, and come into contact with literally any other community on our body which we may touch.

Predictions (hypotheseses):
...
## Sample origin and sequencing
  ...
  The DNA from each of the bacterial communities present was extracted in each sample were sequenced 
...
**Figure 3 Description**: Figure 3 also plots 
...
```{r Proteobacteria Representation Across Samples }
gp.ch = subset_taxa(phyloseq_obj, Phylum == "Proteobacteria")
plot_bar(gp.ch, fill = "Genus", title = "Proteobacteria Genera Distribution Across Samples")
```
...
```{r Bacteriodetes Representation Across Samples}
gp.ch = subset_taxa(phyloseq_obj, Phylum == "Bacteroidetes")
plot_bar(gp.ch, fill = "Genus", title = "Bacteroidetes Genera Distribution Across Samples")
```
...
```{r Firmicutes Representation Across Samples}
gp.ch = subset_taxa(phyloseq_obj, Phylum == "Firmicutes")
plot_bar(gp.ch, fill = "Genus", title = "Firmicutes Genera Distribution Across Samples")
```
...
```{r Cyanobacteria/Chloroplast Across Samples}
gp.ch = subset_taxa(phyloseq_obj, Phylum == "Cyanobacteria/Chloroplast")
plot_bar(gp.ch, fill = "Family", title = "Cyanobacteria/Chloroplast Family Distribution Across Samples")
```
...
```{r Actinobacteria Across Samples}
gp.ch = subset_taxa(phyloseq_obj, Phylum == "Actinobacteria")
plot_bar(gp.ch, fill = "Genus", title = "Actinobacteria Genera Distribution Across Samples")
```
...
```{r Fusobacteria Across Samples}
gp.ch = subset_taxa(phyloseq_obj, Phylum == "Fusobacteria")
plot_bar(gp.ch, fill = "Genus", title = "Fusobacteria Genera Distribution Across Samples")
```
...
```{r Gemmatimonadetes Across Samples}
gp.ch = subset_taxa(phyloseq_obj, Phylum == "Gemmatimonadetes")
plot_bar(gp.ch, fill = "Genus", title = "Gemmatimonadetes Genera Distribution Across Samples")
```
...
Of particular interest, the Cloacibacterium Genera are only present in samples originating from human p
