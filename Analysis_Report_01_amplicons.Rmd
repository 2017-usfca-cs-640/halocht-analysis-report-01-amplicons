...
```{r Preprocess OTU table in phyloseq_obj}
...
```{r Create a second phyloseq obj that does contain phy_tree}

phyloseq_obj_2 <- phyloseq(otu_table(sequence_table_nochim,
                                   taxa_are_rows = FALSE), 
...
PS2.ord <- ordinate(PS2_Top5, "NMDS", "bray")
p1 = plot_ordination(PS2_Top5, PS2.ord, type = "taxa", color_cat = "Phylum", title = "taxa")
print(p1)



top20 <- names(sort(taxa_sums(phyloseq_obj), decreasing=TRUE))[1:20]
ps2.top20 <- transform_sample_counts(phyloseq_obj, function(OTU) OTU/sum(OTU))
ps2.top20 <- prune_taxa(top20, ps2.top20)
plot_bar(ps2.top20, x="Day", fill="Family") + facet_wrap(~When, scales="free_x")
